...
const getBOCFXRatesFromRSSHub = async (
  ...
  jsonData.forEach((item: any) => {
    ...
    if (tmp[0].split("：")[1]) FXRate.rate.buy.remit = parseFloat(tmp[0].split("：")[1]);
    if (tmp[1].split("：")[1]) FXRate.rate.buy.cash = parseFloat(tmp[1].split("：")[1]);
    if (tmp[2].split("：")[1]) FXRate.rate.sell.remit = parseFloat(tmp[2].split("：")[1]);
    if (tmp[3].split("：")[1]) FXRate.rate.sell.cash = parseFloat(tmp[3].split("：")[1]);
...
const getBOCFXRatesFromBOC = async (): Promise<FXRate[]> => {
  const result = await Promise.all(
    [
      ...
      const res = await axios.get(`https://www.boc.cn/sourcedb/whpj/${index}`, {
        headers: {
          "User-Agent": process.env["HEADER_USER_AGENT"] ?? "fxrate axios/latest",
        },
      });
