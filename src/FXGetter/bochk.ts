...
export const getBOCHKFxRates = async (): Promise<FXRate[]> => {
  const result = await Promise.all([getBOCHKFxRatesCash(), getBOCHKFxRatesRemit()]);
  ...
  return currencyList
    .map((k): FXRate => {
      ...
      if (cash && remit)
        updatedTime = cash.updatedDate > remit.updatedDate ? cash.updatedDate : remit.updatedDate;
