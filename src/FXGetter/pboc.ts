...
const getPBOCFXRates = async (): Promise<FXRate[]> => {
  const res = await axios.get("http://www.safe.gov.cn/AppStructured/hlw/RMBQuery.do", {
    headers: {
      "User-Agent":
  ...
  return table.children
    ...
    .map((el) => {
      ...
      const updateTime = new Date($(row.children()[0]).text() + " 00:00 UTC+8");
      ...
      return row
        ...
        .map((thisEL, index): FXRate => {
          ...
          const currencyZHName = $($(table.children[0]).children()[index + 1])
            .text()
            .trim();
          ...
          if (undirectPrice.includes(currencyMap[currencyZHName])) {
            ...
            anz.currency = {
              from: currencyMap[currencyZHName] as currency.unknown,
