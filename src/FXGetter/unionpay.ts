...
const getUnionPayFXRates = async (): Promise<FXRate[]> => {
  let currentDate = parseInt(new Date().toISOString().split("T")[0].replaceAll("-", ""));
  ...
  while (res.status !== 200) {
    ...
    console.log(currentDate + 1, "UnionPay FXRate not found, trying", currentDate);

    res = await axios.get(`https://www.unionpayintl.com/upload/jfimg/${currentDate}.json`, {
      headers: {
  ...
  data.exchangeRateJson.forEach((rate) => {
    ...
    if (isReverse) {
      answerMap[firstCurr][secondCurr].reverse = math.divide(1, rate.rateData);
    } else {
