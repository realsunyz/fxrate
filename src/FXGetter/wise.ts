...
const getWiseFXRates = (
  ...
  return async (fxmManager?: fxmManager): Promise<FXRate[]> => {
    ...
    if (fxmManager && isInSandbox) fxmManager.log("Getting Wise FX Rates in sandbox mode.");
    else if (fxmManager) fxmManager.log("Getting Wise FX Rates in production mode.");
    ...
    for (const rate of data) {
      rate.source = rate.source === "CNY" ? ("CNH" as currency.CNH) : rate.source;
      rate.target = rate.target === "CNY" ? ("CNH" as currency.CNH) : rate.target;
