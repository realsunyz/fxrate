...
export default class fxManager {
  ...
  public async getfxRateList(from: currency, to: currency): Promise<FXRateType> {
  ...
  public update(FXRate: FXRate): void {
    ...
    if (rate.buy.cash) {
      this.fxRateList[from][to].cash = divide(fraction(rate.buy.cash), unit);
    }

    if (rate.sell.cash) {
      this.fxRateList[to][from].cash = divide(unit, fraction(rate.sell.cash));
    }

    if (rate.buy.remit) {
      this.fxRateList[from][to].remit = divide(fraction(rate.buy.remit), unit);
    }

    if (rate.sell.remit) {
      this.fxRateList[to][from].remit = divide(unit, fraction(rate.sell.remit));
    }
  }
  ...
  private async convertDirect(
    ...
    if (!(await this.getfxRateList(from, to))[type]) {
      throw new Error(`FX Path from ${from} to ${to} not support ${type} now`);
    }
    if (reverse) {
      return divide(
        fraction(amount),
  ...
  async getFXPath(from: currency, to: currency): Promise<FXPath> {
    ...
    while (queue.length > 0) {
      ...
      const neighbors = Object.keys(this.fxRateList[currency]) as currency[];
  ...
  async convert(
    ...
    try {
      for (const next of FXPath.path) {
        result = await this.convertDirect(current, next, type, result, reverse);
        current = next;
      }
    } catch (e) {
      throw new Error(`Cannot convert from ${from} to ${to} with ${type}: \n${e.message}`);
    }
