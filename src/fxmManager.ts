...
const formatWithPrecision = (value: number | Fraction, precision: number): string => {
...
export const useInternalRestAPI = async (url: string, router: router) => {
  ...
  const req = new request("GET", u, new interfaces.headers({}), "", {});
...
const useJson = (response: response<any>, request: request<any>): void => {
  ...
  if (request.query.has("pretty") || request.headers.get("Sec-Fetch-Dest") === "document") {
...
const getConvert = async (
  ...
  answer = multiply(answer, 1 + (Number(request.query.get("fees")) || fees) / 100) as Fraction;
...
const getDetails = async (
  ...
  try {
    result.updated = (await fxManager.getUpdatedDate(from, to)).toUTCString();
  } catch (_e) {
...
class fxmManager extends router {
  ...
  public registerGetter(source: string, getter: () => Promise<FXRate[]>): void {
    ...
    this.intervalIDs[source] = {
      timeout: setInterval(() => this.updateFXManager(source), 1000 * 60 * 30),
  ...
  private getFXMRouter(source: string): router {
    ...
    const maybeRefresh = async (request: request<any>) => {
      ...
      if (fxm && typeof fxm.invalidate === "function") {
        const from = request.params?.from ? String(request.params.from).toUpperCase() : undefined;
        const to = request.params?.to ? String(request.params.to).toUpperCase() : undefined;
        try {
    ...
    const invalidTokenPairResponse = async (
      ...
      const body = {
        success: false,
        error: (request as any)?.custom?.turnstile?.error || "token invalid",
        cash: 0,
        remit: 0,
        middle: 0,
        provided: false,
    ...
    const handlerSourceInfo = async (request: request<any>, response: response<any>) => {
      ...
      response.body = JSON.stringify({
        status: "ok",
        source,
        currency: Object.keys((await this.requestFXManager(source)).fxRateList).sort(),
    ...
    const handlerCurrencyAllFXRates = async (request: request<any>, response: response<any>) => {
      if (!isTurnstileValid(request)) {
        response.status = 403;
        response.body = JSON.stringify({
          success: false,
      ...
      await maybeRefresh(request);
      if (request.params.from) request.params.from = request.params.from.toUpperCase();
    ...
    const handlerCurrencyConvert = async (request: request<any>, response: response<any>) => {
      if (request.params.from) request.params.from = request.params.from.toUpperCase();

      if (request.params.to) request.params.to = request.params.to.toUpperCase();
        ...
        if (result.provided === true) {
          response.headers.set(
            ...
            (
              await (
                await this.requestFXManager(source)
              ).getUpdatedDate(from as unknown as currency, to as unknown as currency)
            ).toUTCString(),
          );
        } else {
          response.headers.set("Date", new Date().toUTCString());
    ...
    const handlerCurrencyConvertAmount = async (request: request<any>, response: response<any>) => {
      if (request.params.from) request.params.from = request.params.from.toUpperCase();
      ...
      details.provided = Boolean(
        ...
        (await this.requestFXManager(source)).fxRateList[from][to].provided === true,
      );
    ...
    fxmRouter.binding("/:from", new handler("GET", [handlerSourceInfo, handlerCurrencyAllFXRates]));

    fxmRouter.binding("/:from/:to", new handler("GET", [handlerCurrencyConvert]));

    fxmRouter.binding("/:from/:to/:type", new handler("GET", [handlerCurrencyConvertAmount]));
